...
export default async function AdminSessionsPage() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Sessions"
        title="Monitor de camp"
        description="Seguiment intern de les sessions obertes, tancades i incidencies."
      />

      <SectionCard
        title="Flux actual"
        description="Sessions recents amb l'estat operatiu actual."
      >
        {snapshot.sessions.length === 0 ? (
          <p className="rounded-[22px] border border-dashed border-slate-300 bg-white/60 p-4 text-sm text-slate-500">
            Cap sessio enregistrada encara.
          </p>
        ) : (
